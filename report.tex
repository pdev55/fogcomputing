...
Fog computing provides a solution that is scalable and more secure. Recent camera data is stored at fog gateways located close to the edge for security analysis purposes, and long-term data is sent to the cloud to allow for possible analysis of long-term trends. The addition of an extra layer between the edge and the cloud implies additional security steps in the process of sending data from edge devices to the cloud. Raw camera data is analyzed at the fog gateways, where rules can be established to determine which data must be sent to the cloud and which can be stored locally, and video processing can be partitioned intelligently between the cloud, the fog gateway, and the edge devices.
